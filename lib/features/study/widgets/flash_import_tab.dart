...
class _FlashImportTabState extends ConsumerState<FlashImportTab> {
        ...
        if (_isGenerating) ...[
          Container(
            ...
            child: Column(children: [
              ...
              Text('AI �������ɿ�Ƭ...', style: TextStyle(color: colors.textSecondary)),
            ]),
          ),
          const SizedBox(height: AppSpacing.lg),
        ],
  ...
  Widget _buildEntryCards(AppThemeColors colors) {
    return Row(children: [
      Expanded(child: _EntryCard(icon: Icons.picture_as_pdf_rounded, label: '�ļ�����', subtitle: 'PDF/Word/TXT/MD/ͼƬ', color: colors.study, onTap: _pickAndImportFile)),
      const SizedBox(width: AppSpacing.sm),
      Expanded(child: _EntryCard(icon: Icons.language_rounded, label: '��ҳ����', subtitle: 'URL ץȡ', color: colors.focus, onTap: _showUrlImportDialog)),
      const SizedBox(width: AppSpacing.sm),
      Expanded(child: _EntryCard(icon: Icons.edit_note_rounded, label: '�ı�����', subtitle: '�ֶ�ճ��', color: colors.journal, onTap: _showTextInput)),
    ]);
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: AppSpacing.sm),
          Text('��ʶ������', style: AppTextStyles.sectionTitle.copyWith(color: colors.textPrimary)),
        ]),
        const SizedBox(height: AppSpacing.md),
        if (_parsedTitle != null) _infoRow('����', _parsedTitle!, colors),
        _infoRow('��������', _parsedType == 'paste' ? '�ı�' : _parsedType == 'markdown' ? '��ҳ' : _parsedType, colors),
        if (chunkCount > 0) _infoRow('Ԥ�ƿ�����', '~$estimatedCards �ſ�Ƭ', colors),
      ]),
    );
  }
        ...
        if (_showDetails) ...[
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text('���� ${selected.where((s) => s).length} ��֪ʶ��'),
            ),
          ),
        ],
        const SizedBox(height: AppSpacing.md),
  ...
  Widget _buildAdvancedSettings(AppThemeColors colors) {
    ...
    return Theme(
      ...
      child: ExpansionTile(
        ...
        title: Text('�߼�����', style: AppTextStyles.caption.copyWith(color: colors.textTertiary, fontWeight: FontWeight.w600)),
        children: [
          const SizedBox(height: AppSpacing.sm),
          DropdownButtonFormField<String>(
            initialValue: _selectedGoalKey,
            decoration: const InputDecoration(labelText: '��ϰĿ��'),
          ...
          DropdownButtonFormField<String>(
            ...
            decoration: const InputDecoration(labelText: 'Ŀ����ģ��'),
    ...
    try {
      ...
      try {
        ...
        if (!extractResult.isSuccess) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(extractResult.displayError ?? '�ļ�����ʧ��')));
          return;
        }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('�ļ�����ʧ��: ${e.toString().split("\n").first}')));
      }
    }
  }
  ...
  Future<void> _showUrlImportDialog() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          title: const Text('����ҳ����'),
          content: TextField(controller: _urlController, decoration: const InputDecoration(labelText: '��ҳ��ַ', hintText: 'https://example.com/article'), autofocus: true, keyboardType: TextInputType.url),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('ȡ��')),
            FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('ץȡ')),
          ],
        );
      },
    );
    ...
    try {
      ...
      if (!mounted) return;
      if (!extractResult.isSuccess) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(extractResult.displayError ?? '��ҳץȡʧ��')));
        return;
      }
  ...
  Future<void> _showTextInput() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          backgroundColor: colors.card, surfaceTintColor: colors.card,
          title: const Text('ճ���ı�'),
          content: SizedBox(
            width: double.maxFinite,
            child: TextField(controller: _textController, minLines: 8, maxLines: 15, decoration: const InputDecoration(labelText: 'ѧϰ�����ı�', hintText: 'ճ�����ñʼǡ����������ѧϰ����...', alignLabelWithHint: true)),
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('ȡ��')),
            FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('ȷ��')),
          ],
        );
      },
    );
    ...
    setState(() { _parsedTitle = '�ֶ�����'; _parsedContent = text; _parsedSourcePath = null; _parsedType = 'paste'; _resetDrafts(); });
  }
  ...
  Future<void> _importAndGenerate() async {
    ...
    try {
      ...
      if (source == null || chunks.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('������Ƭʧ�ܣ��������ݺ�����')));
      ...
      if (!mounted) return;
      if (drafts.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('AI û�����ɿ��ÿ�Ƭ���������ݺ�����')));
        setState(() => _isGenerating = false);
        return;
      }
      ...
      if (!mounted) return;
      setState(() => _isGenerating = false);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('����ʧ��: ${e.toString().split("\n").first}')));
    }
  }
  ...
  Future<void> _saveDrafts() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('�ѱ��� $savedCount ��֪ʶ��')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('����ʧ��: ${e.toString().split("\n").first}')));
    }
