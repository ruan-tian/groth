...
class _StudyAnalysisTab extends ConsumerWidget {
  ...
  Future<void> _startStudyAnalysis(WidgetRef ref) async {
    ...
    if (config == null) {
      ref.read(aiAnalysisStateProvider.notifier).updateState(
          const AiAnalysisState(
              error: '未配置 AI 服务，请先在设置中配置 AI API。'));
      return;
    }
    ...
    if (records.isEmpty) {
      ref.read(aiAnalysisStateProvider.notifier).updateState(
          const AiAnalysisState(error: '暂无学习记录可分析。'));
      return;
    }

    final aiService = ref.read(aiServiceProvider);
    await ref.read(aiAnalysisStateProvider.notifier).runAnalysis(
...
class _FitnessAnalysisTab extends ConsumerWidget {
  ...
  Future<void> _startFitnessAnalysis(WidgetRef ref) async {
    ...
    if (records.isEmpty) {
      ref.read(aiAnalysisStateProvider.notifier).updateState(
          const AiAnalysisState(error: '暂无健身记录可分析。'));
      return;
    }
...
class _DietAnalysisTab extends ConsumerWidget {
  ...
  Future<void> _startDietAnalysis(WidgetRef ref) async {
    ...
    if (records.isEmpty) {
      ref.read(aiAnalysisStateProvider.notifier).updateState(
          const AiAnalysisState(error: '暂无饮食记录可分析。'));
      return;
    }
...
class _SleepAnalysisTab extends ConsumerWidget {
  ...
  Future<void> _startSleepAnalysis(WidgetRef ref) async {
    ...
    if (records.isEmpty) {
      ref.read(aiAnalysisStateProvider.notifier).updateState(
          const AiAnalysisState(error: '暂无睡眠记录可分析。'));
      return;
    }
...
class _GrowthReportTab extends ConsumerWidget {
  ...
  Future<void> _generateReport(WidgetRef ref,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ...data.weeklyStats.map((s) => Padding(
