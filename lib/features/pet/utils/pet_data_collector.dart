...
class PetDataCollector {
  ...
  Future<Map<String, dynamic>> collectStudyData() async {
    ...
    for (final r in records) {
      ...
      subjects[subject] = ((subjects[subject] ?? 0) + r.durationMinutes) as int;
    }

    final focusLevels = records.where((r) => r.focusLevel != null).map((r) => r.focusLevel!).toList();
    final avgFocus = focusLevels.isEmpty ? 0.0 : focusLevels.reduce((a, b) => a + b) / focusLevels.length;

    final problems = records.where((r) => r.problem != null && r.problem!.isNotEmpty).map((r) => r.problem!).toList();
  ...
  Future<Map<String, dynamic>> collectFitnessData() async {
    ...
    final fitnessDays = records.map((r) {
      final dt = DateTime.fromMillisecondsSinceEpoch(r.createdAt);
      return '${dt.year}-${dt.month}-${dt.day}';
    }).toSet().length;
    ...
    final intensities = records.where((r) => r.intensityLevel != null).map((r) => r.intensityLevel!).toList();
    final avgIntensity = intensities.isEmpty ? 0.0 : intensities.reduce((a, b) => a + b) / intensities.length;
  ...
  Future<Map<String, dynamic>> collectDietData() async {
    ...
    final totalCalories = records.fold<int>(0, (s, r) {
      switch (r.calorieLevel) {
        case 'low': return s + 300;
        case 'high': return s + 800;
        default: return s + 500;
      }
    });

    final healthScores = records.map((r) => r.healthScore).toList();
    final avgScore = healthScores.isEmpty ? 0.0 : healthScores.reduce((a, b) => a + b) / healthScores.length;
  ...
  Future<Map<String, dynamic>> collectSleepData() async {
    ...
    final avgQuality = qualities.isEmpty ? 0.0 : qualities.reduce((a, b) => a + b) / qualities.length;

    final fallAsleepTimes = records.map((r) => r.fallAsleepMinutes).toList();
    final avgFallAsleep = fallAsleepTimes.isEmpty ? 0.0 : fallAsleepTimes.reduce((a, b) => a + b) / fallAsleepTimes.length;

    final wakeCounts = records.map((r) => r.wakeCount).toList();
    final avgWakeCount = wakeCounts.isEmpty ? 0.0 : wakeCounts.reduce((a, b) => a + b) / wakeCounts.length;
  ...
  Future<Map<String, dynamic>> collectMonthlyReportData() async {
    ...
    final fitnessRecords = await fitnessRepo.getFitnessRecordsByRange(start, now);
    ...
    return {
      ...
      '学习总时长': '${studyRecords.fold<int>(0, (s, r) => s + r.durationMinutes)} 分钟',
      '健身记录数': fitnessRecords.length,
      '健身总时长': '${fitnessRecords.fold<int>(0, (s, r) => s + r.durationMinutes)} 分钟',
