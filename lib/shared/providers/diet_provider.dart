...
final recentDietRecordsProvider =
    FutureProvider.family<List<DietRecord>, int>((ref, limit) async {
  final repo = ref.watch(dietRepositoryProvider);
...
/// 最近30天每日平均健康评分
final monthlyHealthScoreProvider =
    FutureProvider<Map<String, double>>((ref) async {
  final repo = ref.watch(dietRepositoryProvider);
...
/// 根据 ID 获取单条饮食记录
final dietRecordByIdProvider =
    FutureProvider.family<DietRecord?, int>((ref, id) async {
  final repo = ref.watch(dietRepositoryProvider);
...
final dailyCalorieTotalsProvider =
...
final dailyCalorieWaterProvider =
  ...
  if (waterValue != null) {
    try {
      final map = (jsonDecode(waterValue) as Map<String, dynamic>)
