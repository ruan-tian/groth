...
import 'package:growth_os/features/fitness/utils/fitness_timer_assets.dart';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  testWidgets('FitnessTimerAssets are bundled and loadable', (tester) async {
    expect(FitnessTimerAssets.all, hasLength(17));

    for (final asset in FitnessTimerAssets.all) {
      final data = await rootBundle.load(asset);
      expect(data.lengthInBytes, greaterThan(0), reason: asset);
    }
  });

  testWidgets('FitnessTimerAssets have alpha or are WebP', (
    tester,
  ) async {
    for (final asset in FitnessTimerAssets.all) {
      final data = await rootBundle.load(asset);
